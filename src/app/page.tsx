...
export default function Home() {
    ...
    setTransactions((prev) =>
      prev.map((tx) =>
        tx.id === editingTransaction.id
          ? { ...tx, amount, type, category, memo, paymentMethod }
          : tx,
      ),
    );
    setIsEditOpen(false);
  ...
  const selectedDateTransactions = transactions.filter((tx) =>
    isSameDay(tx.date, selectedDate),
  );
              ...
              {selectedDateTransactions.map((tx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p
                      className={`text-[17px] font-extrabold ${tx.type === "income" ? "text-primary" : "text-foreground"}`}
                    >
                      {tx.type === "income" ? "+" : ""}
